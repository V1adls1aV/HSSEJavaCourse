...
public interface UserRepository extends JpaRepository<User, Long> {
  ...
  Optional<User> findUserWithBooks(@Param("userId") Long userId);

  @Query("SELECT u FROM User u LEFT JOIN FETCH u.courses WHERE u.id = :userId")
  Optional<User> findUserWithCourses(@Param("userId") Long userId);

  @Query("SELECT u FROM User u LEFT JOIN FETCH u.universities WHERE u.id = :userId")
  Optional<User> findUserWithUniversities(@Param("userId") Long userId);
}
